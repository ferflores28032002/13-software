...
import { useUpdateUser } from "@/hooks/users/useUpdateUser"; // Importa el hook de actualización
...
export const columns: ColumnDef<User>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const [isDialogOpen, setIsDialogOpen] = useState(false); // Controla la apertura del modal

      const queryClient = useQueryClient(); // Usa el queryClient para invalidar queries

      const updateUserMutation = useUpdateUser(); // Hook para actualizar usuarios

      const handleEditSubmit = (formData: UserFormInputs) => {
        const payload = {
          ...formData,
          password: formData.password || "", // El campo password es opcional
          id: row.original.id, // Asegura que el ID se incluye en la actualización
        };
